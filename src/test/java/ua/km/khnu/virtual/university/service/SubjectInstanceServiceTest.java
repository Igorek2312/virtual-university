...
public class SubjectInstanceServiceTest {
    ...
    public void getSemesters() throws Exception {
        ...
        when(subjectInstanceRepository.findByGroupId(1, any(Sort.class))).thenReturn(subjectInstances);
        List<SemesterDto> semesters = subjectInstanceService.getSemesters(1);

        assertThat(semesters)
                .extracting(SemesterDto::getYear)
                .containsExactly(2017, 2016);

        assertThat(semesters)
                .extracting(SemesterDto::getSemesterNumber)
                .containsExactly(2, 1);
